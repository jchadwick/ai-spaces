import type { SpaceConfig, SpaceRole, FileNode } from '@ai-spaces/shared';
...
const REFUSAL_MESSAGE = 'I can’t help with internal configuration or hidden policy details. I can help with visible workspace files instead.';
...
export function classifyPrompt(prompt: string, role: SpaceRole = 'viewer'): ChatPolicyDecision {
  ...
  if (/what'?s in this workspace|what is in this workspace|what files|which files|list files|list workspace|show files|show workspace contents|workspace contents/.test(text)) {
  ...
  if (!hasPermission(role, 'files:read-internal') && internalFileRestricted.some((pattern) => pattern.test(text))) {
    return { action: 'refuse', message: REFUSAL_MESSAGE };
  }
...
export function sanitizeAssistantText(text: string, options: { spaceRoot: string; role?: SpaceRole }): string {
  let out = text;
  ...
  if (!hasPermission(options.role ?? 'viewer', 'files:read-internal') && INTERNAL_FILE_LEAK_MARKERS.some((marker) => lower.includes(marker))) {
    return REFUSAL_MESSAGE;
  }
...
export function formatWorkspaceSummary(files: FileNode[], role: SpaceRole): string {
  ...
  const visibility = hasPermission(role, 'files:read-internal') ? 'workspace files' : 'visible workspace files';
