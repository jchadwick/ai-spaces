...
async function installRoomsContentMocks(page: Page) {
  ...
  await page.route(/^https?:\/\/[^/]+\/api\/.*/, async (route) => {
    ...
    if (method === 'GET' && path === `/api/spaces/${SPACE_ID}/topics/session`) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ topic: null }) });
      return;
    }
    ...
    if (path.startsWith(filePrefix)) {
      ...
      if (method === 'PUT') {
        ...
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, path: filePath }) });
        return;
      }
    }

    await route.fulfill({ status: 404, contentType: 'application/json', body: JSON.stringify({ error: `Unhandled mock route: ${method} ${path}` }) });
  });
...
test.describe('Rooms content viewer registry', () => {
  test('renders markdown and plain text, then saves both from the visible room file pane', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Markdown registry audit' })).toBeVisible({ timeout: 5000 });
    await expect(page.getByText('viewer assertion')).toBeVisible();

    await page.getByRole('button', { name: 'Edit', exact: true }).click();
    await page.locator('textarea:not([placeholder="Reconnecting..."])').first().fill('# Markdown registry audit\n\nEdited markdown through visible UI.\n');
    ...
    await expect(page.locator('pre').filter({ hasText: 'Plain text registry audit' })).toBeVisible({ timeout: 5000 });
