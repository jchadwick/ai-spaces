...
describe("chat policy", () => {
  ...
  it("redacts absolute workspace path from assistant text", () => {
    const out = sanitizeAssistantText("Root: /tmp/workspace/proj", {
      spaceRoot: "/tmp/workspace/proj",
    });
  ...
  it("replaces leaked file inventory output with refusal", () => {
    const out = sanitizeAssistantText(
      "Here are files in /home/openclaw/workspace:\nAGENTS.md\n.space/chat-history.json",
      { spaceRoot: "/home/openclaw/workspace/TestSpace" },
    );
    expect(out.toLowerCase()).toContain("internal configuration");
  ...
  it("removes internal files from trees for non-owner listings", () => {
    const files = removeInternalFiles([
      { name: "AGENTS.md", path: "AGENTS.md", type: "file" },
      {
        ...
        path: ".space",
        type: "directory",
        children: [{ name: "chat-history.json", path: ".space/chat-history.json", type: "file" }],
      },
      { name: "CostaRica.md", path: "CostaRica.md", type: "file" },
      {
        ...
        path: "home",
        type: "directory",
        children: [{ name: "AGENTS.md", path: "home/openclaw/workspace/AGENTS.md", type: "file" }],
      },
    ]);
