...
import { authProviders, serverRoles, servers, spaceMembers, spaces, users } from "./index.js";
...
export function getServerBySpaceId(spaceId: string): typeof servers.$inferSelect | null {
  const row = db
    .select({ serverId: spaces.serverId })
    .from(spaces)
...
export function getUserSpaceRoles(userId: string, spaceIds: string[]): Map<string, SpaceRole> {
  ...
  const spaceRows = db
    .select({ id: spaces.id, serverId: spaces.serverId })
    .from(spaces)
  ...
  const isGodAdmin =
    db
      ...
      .where(
        and(
          eq(serverRoles.userId, userId),
          eq(serverRoles.serverId, DEFAULT_SERVER_ID),
          eq(serverRoles.role, "admin"),
        ),
      )
      .get()?.role === "admin";
  ...
  if (isGodAdmin) {
    ...
    const adminRows = db
      .select({ serverId: serverRoles.serverId })
      .from(serverRoles)
