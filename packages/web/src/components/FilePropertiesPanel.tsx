...
export function FilePropertiesPanel({ spaceId, filePath, fileInfo, onClose }: FilePropertiesPanelProps) {
    ...
    <div style={panelStyle}>
      <div style={{ display: 'flex', gap: 16 }}>
        ...
        {fileInfo?.type && (
          <div style={{ minWidth: 90 }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--t-inkMid)', padding: '6px 0', fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}>
              {fileInfo.type}
            </div>
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
        <button
          ...
          onClick={onClose}
          disabled={saving}
          style={{ background: 'none', border: '1px solid var(--t-hair)', borderRadius: 6, padding: '5px 14px', fontSize: 13, color: 'var(--t-inkMid)', cursor: 'pointer' }}
        >
          Cancel
        </button>
        <button
