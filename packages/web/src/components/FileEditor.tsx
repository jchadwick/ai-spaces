...
function saveDraft(spaceId: string, filePath: string, content: string) {
  try {
    localStorage.setItem(getDraftKey(spaceId, filePath), JSON.stringify({ content, savedAt: new Date().toISOString() }))
...
function clearDraft(spaceId: string, filePath: string) {
  try { localStorage.removeItem(getDraftKey(spaceId, filePath)) } catch { /* ignore */ }
}
...
export default function FileEditor({
  ...
  const handleSave = useCallback(async () => {
    if (!filePath || !spaceId) { setSaveError('Cannot save: missing file or space'); return }
    setIsSaving(true)
    setSaveError(null)
    try {
  ...
  const handleCommitRename = useCallback(async () => {
    ...
    if (!trimmed || trimmed === fileInfo.name) { handleCancelRename(); return }
      ...
      <section className="flex-1 flex flex-col bg-surface-container-lowest overflow-hidden">
        {showConcurrentWarning && (
          <div className="bg-warning-container border-b border-warning text-on-warning-container px-4 py-2 flex items-center gap-2">
            ...
            <button type="button" onClick={() => setShowConcurrentWarning(false)} className="ml-auto text-on-warning-container hover:opacity-75">
