...
import { confirmationNonces, spaceMembers, inviteTokens, notifications } from '../db/index.js';
...
confirmRouter.post(
  ...
  async (c) => {
    ...
    db.insert(confirmationNonces).values({
      id,
      spaceId,
      issuingUserId,
      action,
...
confirmRouter.post(
  ...
  async (c) => {
    ...
    const nonce = db.select().from(confirmationNonces).where(eq(confirmationNonces.id, nonceId)).get();
