...
function parseHttpUrl(value: string | undefined, name: string, fallback: string): string {
  ...
  try {
    ...
    if (!['http:', 'https:'].includes(parsed.protocol)) {
      markInvalid(name, `${name} must use http or https, got '${parsed.protocol}'. Falling back to '${fallback}'.`);
      return fallback;
    }
    return candidate;
  } catch {
    markInvalid(name, `${name} must be a valid URL, got '${candidate}'. Falling back to '${fallback}'.`);
    return fallback;
  }
}
...
function parseAbsolutePath(value: string | undefined, name: string, fallback: string): string {
  ...
  if (!path.isAbsolute(candidate)) {
    markInvalid(name, `${name} must be an absolute path, got '${candidate}'. Falling back to '${fallback}'.`);
    return fallback;
  }
  return candidate;
}
...
function parsePort(value: string | undefined, name: string, fallback: number): number {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0 || parsed > 65535) {
    markInvalid(name, `${name} must be a valid port (1-65535), got '${candidate}'. Falling back to '${fallback}'.`);
    return fallback;
  }
  return parsed;
}
...
function parsePositiveNumber(value: string | undefined, name: string, fallback: number): number {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    markInvalid(name, `${name} must be a positive number, got '${candidate}'. Falling back to '${fallback}'.`);
    return fallback;
  }
  return parsed;
}
...
export const config = {
  ...
  FILE_STREAM_THRESHOLD_MB: parsePositiveNumber(process.env.FILE_STREAM_THRESHOLD_MB, 'FILE_STREAM_THRESHOLD_MB', 1),
};

if (!config.GATEWAY_TOKEN) {
  markInvalid('GATEWAY_TOKEN', 'GATEWAY_TOKEN is missing; registration/reconcile and auth-dependent callbacks will run in degraded mode.');
}
