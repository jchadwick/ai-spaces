...
import { type AuditEntry, useAuditLog } from "@/hooks/useAuditLog";
  ...
  return (
    <div className="flex items-start gap-3 py-2 px-2 rounded-md hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-medium text-t-ink truncate">{label}</p>
        {entry.spaceId && (
          <p className="text-xs text-t-ink-dim truncate">Space: {entry.spaceId.slice(0, 8)}...</p>
        )}
        <p className="text-xs text-t-ink-dim/70">{formatTime(entry.timestamp)}</p>
      </div>
    </div>
  );
}
