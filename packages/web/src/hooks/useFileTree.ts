...
export function useFileTree(spaceId: string | undefined): FileTree {
  ...
  const fetchDir = useCallback(
    async (sid: string, dirPath: string): Promise<FileNode[]> => {
  ...
  const prefetchAllRef = useRef<(sid: string, nodes: FileNode[]) => Promise<void>>(() =>
    Promise.resolve(),
  );

  const prefetchAll = useCallback(
    async (sid: string, nodes: FileNode[]) => {
