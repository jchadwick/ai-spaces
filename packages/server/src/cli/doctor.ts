...
async function checkPort(): Promise<CheckResult> {
  return new Promise((resolve) => {
    ...
    srv.once("error", () =>
      resolve(warn("Port available", `${config.AI_SPACES_PORT} — may already be in use`)),
    );
    srv.once("listening", () =>
      srv.close(() => resolve(pass("Port available", String(config.AI_SPACES_PORT)))),
    );
...
async function checkServerConnectivity(): Promise<CheckResult> {
  try {
    ...
    return warn(
      "Server /health",
      `${config.AI_SPACES_URL} — unreachable (${err instanceof Error ? err.message : String(err)})`,
    );
  }
}
...
async function checkPluginConnectivity(): Promise<CheckResult | null> {
  ...
  try {
    ...
    return warn(
      "Plugin /health",
      `${config.PLUGIN_URL} — unreachable (${err instanceof Error ? err.message : String(err)})`,
    );
  }
}
