...
function buildOpenClawInstallScript(serverUrl: string, registrationToken: string): string {
  ...
  return [
    ...
    `INSTALLED_VERSION=""`,
    `if openclaw plugins list 2>/dev/null | grep -qi "ai-spaces"; then`,
    `  INSTALLED_VERSION="$(openclaw plugins list 2>/dev/null | grep -i "ai-spaces" | awk '{print $NF}' || true)"`,
    `fi`,
    ``,
    `if [ -n "${S("INSTALLED_VERSION")}" ] && [ "${S("INSTALLED_VERSION")}" = "${S("LATEST_VERSION")}" ]; then`,
    `  log "Plugin v${S("INSTALLED_VERSION")} is already up to date."`,
    `else`,
    `  if [ -n "${S("INSTALLED_VERSION")}" ]; then`,
    `    log "Upgrading from v${S("INSTALLED_VERSION")} to v${S("LATEST_VERSION")}"`,
    `  else`,
    ...
    `  curl -fsSL "${S("ARTIFACT_URL")}" -o "${S("WORKDIR")}/${S("ARTIFACT_FILENAME")}" \\`,
    ...
    `  tar -xzf "${S("WORKDIR")}/${S("ARTIFACT_FILENAME")}" -C "${S("WORKDIR")}"`,
    `  openclaw plugins install --link "${S("WORKDIR")}/openclaw-spaces" \\`,
