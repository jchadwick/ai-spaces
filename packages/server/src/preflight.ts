...
export async function runPreflightChecks(): Promise<void> {
  ...
  if (process.env.NODE_ENV === 'production' && config.JWT_SECRET === DEV_JWT_SECRET) {
    throw new Error('Preflight FAIL: JWT_SECRET is set to the insecure development default in production');
  }

  // Warn if web dist is missing
  if (!fs.existsSync(config.WEB_DIST)) {
    log.warn({ dir: config.WEB_DIST }, 'Preflight WARN: WEB_DIST directory not found — UI will not be served');
  }
