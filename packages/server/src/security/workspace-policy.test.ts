...
describe('WorkspacePolicy', () => {
  ...
  it.each([
    ...
    ['symlink escape', { ...visibleFile, requestedPath: 'linked/passwd', contained: false, symlinkEscaped: true }],
  ])('rejects %s facts', async (_label, facts) => {
    await expect(policyFor(facts).approvePath(space, facts.requestedPath)).rejects.toThrow('outside workspace');
  });

  it('rejects hidden paths unless Hono explicitly allows internal access', async () => {
    const hidden = { ...visibleFile, requestedPath: '.space/SPACE.md', canonicalRelativePath: '.space/SPACE.md', hidden: true };
    await expect(policyFor(hidden).approvePath(space, hidden.requestedPath)).rejects.toThrow('hidden path');
    await expect(policyFor(hidden).approvePath(space, hidden.requestedPath, { allowHidden: true })).resolves.toMatchObject({
      path: '.space/SPACE.md',
    });
  });
});
