...
import { spaceTopics, type SpaceTopic } from '../db/index.js';
...
export function getTopic(spaceId: string, topicPath: string): SpaceTopic | undefined {
  return db.select().from(spaceTopics)
    .where(and(eq(spaceTopics.spaceId, spaceId), eq(spaceTopics.topicPath, normalizeTopicPath(topicPath))))
    .get();
}
...
export function persistTopicSession(
  ...
  if (normalized === '/') {
    db.insert(spaceTopics).values({
      ...
      target: [spaceTopics.spaceId, spaceTopics.topicPath],
      set: { acpSessionId, targetType: 'root', status: 'active', archivedAt: null, updatedAt: now },
    }).run();
  } else {
    const topic = getActiveTopic(spaceId, normalized);
    if (!topic) throw new Error('Topic is not active');
    db.update(spaceTopics).set({ acpSessionId, updatedAt: now })
...
export function archiveTopicTree(spaceId: string, topicPath: string): void {
  ...
  for (const topic of db.select().from(spaceTopics).where(eq(spaceTopics.spaceId, spaceId)).all()) {
    if (topic.topicPath === normalized || topic.topicPath.startsWith(`${normalized}/`)) {
      db.update(spaceTopics).set({ status: 'archived', archivedAt: now, updatedAt: now })
        .where(eq(spaceTopics.id, topic.id)).run();
    }
  }
}
...
export function renameTopicTree(spaceId: string, fromPath: string, toPath: string): void {
  ...
  const now = new Date().toISOString();
  const topics = db.select().from(spaceTopics).where(eq(spaceTopics.spaceId, spaceId)).all()
  ...
  for (const topic of topics) {
    ...
    db.update(spaceTopics).set({ topicPath: `${to}${suffix}`, updatedAt: now })
      .where(eq(spaceTopics.id, topic.id)).run();
  }
}
